...
internal class BlockTable(private val zcashNetwork: ZcashNetwork, private val sqliteDatabase: SQLiteDatabase) {
    companion object {

        private val SELECTION_MIN_HEIGHT = String.format(
            Locale.ROOT,
            "MIN(%s)", // $NON-NLS
            BlockTableDefinition.COLUMN_LONG_HEIGHT
        )

        private val SELECTION_MAX_HEIGHT = String.format(
            Locale.ROOT,
            "MAX(%s)", // $NON-NLS
            BlockTableDefinition.COLUMN_LONG_HEIGHT
        )
    ...
    suspend fun firstScannedHeight(): BlockHeight {
        ...
        val heightLong =
            sqliteDatabase.queryAndMap(
                ...
                selection = SELECTION_MIN_HEIGHT,
                cursorParser = { it.getLong(0) }
            ).first()
    ...
    suspend fun lastScannedHeight(): BlockHeight {
        ...
        val heightLong =
            sqliteDatabase.queryAndMap(
                table = BlockTableDefinition.TABLE_NAME,
                selection = SELECTION_MAX_HEIGHT,
                cursorParser = { it.getLong(0) }
            ).first()
    ...
    suspend fun findBlockHash(blockHeight: BlockHeight): ByteArray? {
        return sqliteDatabase.queryAndMap(
            ...
            selectionArgs = arrayOf(blockHeight.value.toString()),
