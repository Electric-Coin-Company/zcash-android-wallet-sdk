...
object SaplingParamTool {
    ...
    suspend fun fetchParams(destinationDir: String) {
        var failureMessage = ""
        arrayOf(
            SaplingFileParameters(ZcashSdk.SPEND_PARAM_FILE_NAME, SPEND_PARAM_FILE_MAX_BYTES_SIZE),
            SaplingFileParameters(ZcashSdk.OUTPUT_PARAM_FILE_NAME, OUTPUT_PARAM_FILE_MAX_BYTES_SIZE)
        ).forEach { parameters ->
