...
1. Run the Gradle task `flankAuth` to generate a Firebase authentication token on your machine.  Note that Gradle may report the task failed yet still successfully store the token
