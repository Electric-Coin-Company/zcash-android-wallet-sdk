...
/**
 * Wrapper for librustzcash. This class basically represents all the Rust-wallet capabilities and the supporting data
...
class Wallet(
    ...
    private val librustzcash: LibrustzcashWelding,
    ...
    constructor(
        context: Context,
        librustzcash: LibrustzcashWelding,
        dataDbPath: String,
    ...
    fun initialize(
        ...
        librustzcash.initDataDb(dataDbPath)
        twig("seeding the database with sapling tree at height ${birthday.height}")
        librustzcash.initBlocksTable(dataDbPath, birthday.height, birthday.hash, birthday.time, birthday.tree)

        // store the spendingkey by leveraging the utilities provided during construction
        val seed by seedProvider
        val accountSpendingKeys = librustzcash.initAccountsTable(dataDbPath, seed, 1)
    ...
    fun getAddress(accountId: Int = accountIds[0]): String {
        return librustzcash.getAddress(dataDbPath, accountId)
    }
    ...
    fun availableBalanceSnapshot(accountId: Int = accountIds[0]): Long {
        return librustzcash.getVerifiedBalance(dataDbPath, accountId)
    }
    ...
    suspend fun sendBalanceInfo(accountId: Int = accountIds[0]) = withContext(IO) {
        twigTask("checking balance info") {
            try {
                val balanceTotal = librustzcash.getBalance(dataDbPath, accountId)
                twig("found total balance of: $balanceTotal")
                val balanceAvailable = librustzcash.getVerifiedBalance(dataDbPath, accountId)
